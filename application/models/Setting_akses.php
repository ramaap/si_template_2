...
class setting_akses extends CI_Model {
	...
	{
		$this->db->select("*,b.role_nama as role_nama,a.am_id as datamodel");
        $this->db->from('setting_akses_menu a');
        $this->db->join('data_role b','b.role_id=a.role_id and b.is_delete=0');
		$this->db->where('a.is_delete', '0');
	...
	public function get_pass_javascript_by_id($akses)
	{
		$data=$this->db->query('select SUBSTRING_INDEX(SUBSTRING_INDEX('.$akses.'_aksi, "~", 4), "~", -1) as akses,user_password from setting_akses_menu a
		join data_role b on b.role_id=a.role_id 
		join data_user c on b.role_id=c.role_id 
		having akses=1
		')->result();
		 return $data;
		// SUBSTRING_INDEX(SUBSTRING_INDEX(membername, ' ', 1), ' ', -1) as memberfirst
	}
